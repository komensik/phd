...
write.csv(public_safety_table_total, "pub_safety_w_total.csv", row.names = FALSE)
```
...
needed_root <- c(
  "state_demographic_data.csv",
  "state_hispanic_pop_data.csv",
  "shor mccarty 1993-2016 state aggregate data May 2018 release (Updated July 2018).dta",
  "data_onepartystates.dta",
  "cost_of_voting_index.csv",
  "epi_indicators-all_years.csv"
)

needed_output <- c(
  "output/democracy_measures_paper.csv",
  "output/democracy_full_factanal.csv"
)

missing_root <- needed_root[!file.exists(needed_root)]
missing_output <- needed_output[!file.exists(needed_output)]
...
data %>%
  select(state, year, blackincarcerate, det_sentencing, residual) %>%
  filter(year == 2018)


subset(data, year == 2018) %>%
  select(state, democracy_mcmc_lib)
```
