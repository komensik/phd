...
get_diff_table <- function(df, group_var) {
  df %>%
    dplyr::filter(!is.na(deserving)) %>%
    dplyr::group_by(target, !!rlang::sym(group_var)) %>%
    dplyr::summarise(mean = mean(deserving, na.rm = TRUE), .groups = "drop") %>%
...
impact_tab <- get_diff_table(long_df, "impact")
incarc_tab <- get_diff_table(long_df, "incarc")
know_tab   <- get_diff_table(long_df, "knowincarc")
vict_tab   <- get_diff_table(long_df, "vict")

get_pvals_impact <- function(df) {
  df %>%
    dplyr::filter(!is.na(deserving), !is.na(impact)) %>%
    dplyr::group_by(target) %>%
    dplyr::summarise(
      pval = t.test(deserving ~ impact)$p.value,
      .groups = "drop"
    )
}
...
impact_pvals <- get_pvals_impact(long_df)
...
# -----------------------------
make_plot_df <- function(df, xvar, subgroup_var, subgroup_label, target_label) {
  df %>%
    transmute(
      deserving = .data[[xvar]],
      group = case_when(
...
### 2.001 Attemting a big Deservingness Means Loop
...
impact_labels <- c(
  impact = "Any impact",
  incarc = "Ever incarcerated",
  knowincarc = "Knows incarcerated person",
  vict = "Victim"
)
...
dvs <- c("prisonhelp", "prisonpen", "prisonpen_1", "prisonpen_5",
         "q23_2_rev", "q23_7_rev", "q25_2_rev",
         "q25_3_rev", "q25_6_rev", "q25_4_rev")

ivs <- c("impact", "incarc", "knowincarc", "vict")
