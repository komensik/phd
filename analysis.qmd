...
policy_dummies_models <- list()

for (dv in policy_vars) {
  for (iv in impact_vars) {
    name <- paste0("m_", dv, "_", iv)
    policy_dummies_models[[name]] <- lm(
...
make_comparison_model <- function(df, dv, subgroup_var) {
  df_sub <- df %>%
    filter(impact == 0 | .data[[subgroup_var]] == 1) %>%
    mutate(treat = ifelse(.data[[subgroup_var]] == 1, 1, 0))

  lm(as.formula(paste(dv, "~ treat")), data = df_sub) #fits DV to treat to compare non-impacted to incarc, knowincarc, vict
}

policy_vs_nonimp_nocontrol_models <- list()

# any impact vs no impact
for (dv in policy_vars) {
  policy_vs_nonimp_nocontrol_models[[paste0("comp_", dv, "_impact")]] <-
    lm(as.formula(paste(dv, "~ impact")), data = raw_data_cleaned)
}

# each specific subgroup vs non-impacted only
for (dv in policy_vars) {
  for (iv in c("incarc", "knowincarc", "vict")) {
    name <- paste0("comp_", dv, "_", iv)
    policy_vs_nonimp_nocontrol_models[[name]] <- make_comparison_model(raw_data_cleaned, dv, iv)
  }
}
