...
#delete this: average time
```{r}
summary(raw_data_cleaned$duration_in_seconds)

quantile(raw_data_cleaned$duration_in_seconds,
         probs = c(.25, .5, .75, .9, .95, .99), na.rm = TRUE)
hist(raw_data_cleaned$duration_in_seconds, breaks = 100)
```
...
### 1.01 Policy Questions Doing Cronback's Alpha Late
...
pen_items <- raw_data_cleaned %>%
  dplyr::select(prisonpen_1, prisonpen_5)

# Quick look
psych::describe(pen_items)

# Correlation (more interpretable than alpha for 2 items)
cor(pen_items, use = "pairwise.complete.obs")

# Cronbach's alpha (fine to report, but remember it's 2 items)
psych::alpha(pen_items, check.keys = FALSE)
...
-   The **correlation** between the two items is **r = 0.238**.
...
### Interpretation to stand behind

-   **These two punitive items are positively related, but only weak-to-moderately so.** People who favor the death penalty are *somewhat* more likely to favor LWOP, but plenty of respondents don’t treat them as interchangeable.
...
### What I’d do next (briefly)

-   

-   Keep the 2-item average as a *simple punitive severity index*, but be transparent that it’s two items with modest correlation.

-   

-   As a robustness check later: re-run the correlation/alpha **excluding DK** (treat DK as NA) and confirm the relationship is similar.

### Alpha for Prison Help

```{r}
help_items <- raw_data_cleaned %>%
  select()

psych::describe(help_items)

help_items <- raw_data_cleaned %>%
  dplyr::select(q25_2_rev, q25_3_rev, q25_4_rev, q25_6_rev, q23_2_rev, q23_7_rev)

help_alpha <- psych::alpha(help_items, check.keys = FALSE)
help_alpha$total$raw_alpha      # the alpha value
help_alpha$item.stats           # item stats
help_alpha$alpha.drop           # alpha if item dropped
```
...
tapply(raw_data_cleaned$d_incppl_pris_abc,
       raw_data_cleaned$impact,
...
### criminals
tapply(raw_data_cleaned$d_crim_abc,
       raw_data_cleaned$impact,
       function(x) c(mean = mean(x, na.rm = TRUE),
                     var  = var(x, na.rm = TRUE),
                     sd   = sd(x, na.rm = TRUE)))
...
### incarcerated women
tapply(raw_data_cleaned$d_incwomen_abc,
       raw_data_cleaned$impact,
       function(x) c(mean = mean(x, na.rm = TRUE),
                     var  = var(x, na.rm = TRUE),
                     sd   = sd(x, na.rm = TRUE)))
...
raw_data_cleaned$deserve_bin <- cut(
  raw_data_cleaned$d_incppl_pris_abc,
...
prop.table(
  table(raw_data_cleaned$deserve_bin, raw_data_cleaned$impact),
  margin = 2
)

prop.table(
  table(raw_data_cleaned$deserve_bin, raw_data_cleaned$incarc),
  margin = 2
)

prop.table(
  table(raw_data_cleaned$deserve_bin, raw_data_cleaned$knowincarc),
  margin = 2
)
prop.table(
  table(raw_data_cleaned$deserve_bin, raw_data_cleaned$vict),
  margin = 2
)
round(
  prop.table(table(raw_data_cleaned$deserve_bin, raw_data_cleaned$impact), 2) * 100,
  1
)
tab <- prop.table(
  table(raw_data_cleaned$deserve_bin, raw_data_cleaned$impact),
...
ggplot(raw_data_cleaned, aes(x = d_incppl_pris_abc, fill = factor(impact))) +
  geom_histogram(position = "identity", alpha = 0.5, bins = 20) +
  labs(
    x = "Deservingness toward Incarcerated People",
    y = "Count",
    fill = "Impact",
    title = "Distribution of Deservingness toward Incarcerated People by Carceral Impact"
  ) +
  theme_minimal()

ggplot(raw_data_cleaned, aes(x = d_incppl_pris_abc, fill = factor(incarc))) +
  geom_histogram(position = "identity", alpha = 0.5, bins = 20) +
  labs(
    x = "Deservingness toward Incarcerated People",
    y = "Count",
    fill = "Incarcerated",
    title = "Distribution of Deservingness by Personal Incarceration"
  ) +
  theme_minimal()

ggplot(raw_data_cleaned, aes(x = d_incppl_pris_abc, fill = factor(knowincarc))) +
  geom_histogram(position = "identity", alpha = 0.5, bins = 20) +
  labs(
    x = "Deservingness toward Incarcerated People",
    y = "Count",
    fill = "Knows Incarcerated",
    title = "Distribution of Deservingness by Knowing Someone Incarcerated"
  ) +
  theme_minimal()

ggplot(raw_data_cleaned, aes(x = d_incppl_pris_abc, fill = factor(vict))) +
  geom_histogram(position = "identity", alpha = 0.5, bins = 20) +
  labs(
    x = "Deservingness toward Incarcerated People",
    y = "Count",
    fill = "Victim",
    title = "Distribution of Deservingness by Victimization Experience"
  ) +
  theme_minimal()

ggplot(raw_data_cleaned, aes(x = d_incppl_pris_abc, color = factor(impact))) +
  geom_density(size = 1.2) +
  labs(
    x = "Deservingness toward Incarcerated People",
    y = "Density",
    color = "Impact",
    title = "Distribution of Deservingness toward Incarcerated People by Impact"
  ) +
  theme_minimal()
...
plot_df <- raw_data_cleaned %>%
  transmute(
    d = d_incppl_pris_abc,
    baseline_impact0 = impact == 0,
    incarc1 = incarc == 1,
    knowincarc1 = knowincarc == 1,
    vict1 = vict == 1
  ) %>%
  pivot_longer(
    cols = c(baseline_impact0, incarc1, knowincarc1, vict1),
    names_to = "group",
    values_to = "in_group"
  ) %>%
  filter(in_group, !is.na(d)) %>%
  mutate(
    group = recode(group,
      baseline_impact0 = "Not impacted (impact = 0)",
      incarc1 = "Ever incarcerated (incarc = 1)",
      knowincarc1 = "Knows incarcerated (knowincarc = 1)",
      vict1 = "Victim (vict = 1)"
    )
  )
...
ggplot(plot_df, aes(x = d, color = group)) +
  ...
  labs(
    ...
    y = "Density",
    color = NULL,
    title = "Deservingness Distributions: Not Impacted vs Impact Subgroups"
  ) +
  theme_minimal()

#side by side

xlim_common <- range(raw_data_cleaned$d_incppl_pris_abc, na.rm = TRUE)

density_compare <- function(df, subgroup_var, subgroup_label, xvar = "d_incppl_pris_abc", xlim = c(0,100)) {
  
  plot_df <- df %>%
    transmute(
      d = .data[[xvar]],
      group = case_when(
        impact == 0 ~ "Not impacted (impact = 0)",
...
p_impact  <- density_compare(raw_data_cleaned, "impact",
                            "Impacted (impact = 1)", xlim = xlim_common)

p_incarc  <- density_compare(raw_data_cleaned, "incarc",
                            "Ever incarcerated (incarc = 1)", xlim = xlim_common)

p_know    <- density_compare(raw_data_cleaned, "knowincarc",
                            "Knows incarcerated (knowincarc = 1)", xlim = xlim_common)

p_vict    <- density_compare(raw_data_cleaned, "vict",
                            "Victim (vict = 1)", xlim = xlim_common)

# combine plots
combined_plot <- (p_impact | p_incarc | p_know | p_vict) +
...
# save and export
ggsave(
  filename = "deservingness_density_by_contact.png",
  plot = combined_plot,
  width = 14,
  height = 4,
  dpi = 300
...
```{r}
help_items <- c("q25_2_rev", "q25_3_rev", "q25_4_rev", "q25_6_rev")

group_vars <- c("impact", "incarc", "knowincarc", "vict")

summ_by_group <- function(df, y, g){
  tapply(df[[y]], df[[g]], function(x) c(
...
for (y in help_items) {
  ...
  for (g in group_vars) {
    cat("\n--- Grouped by:", g, "---\n")
    print(summ_by_group(raw_data_cleaned, y, g))
  }
}
...
# punitive items (cleaned versions)
pun_items <- c("q25_1", "q25_5")

# grouping variables
group_vars <- c("impact", "incarc", "knowincarc", "vict")

# helper function: mean/var/sd by group
summ_by_group <- function(df, y, g){
  tapply(df[[y]], df[[g]], function(x) c(
    mean = mean(x, na.rm = TRUE),
    var  = var(x, na.rm = TRUE),
...
# quick sanity check (should print character(0) twice)
setdiff(pun_items, names(raw_data_cleaned))
setdiff(group_vars, names(raw_data_cleaned))
