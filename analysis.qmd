...
for (dv in policy_vars) {
  for (iv in impact_vars) {
    ...
    policy_dummies_models[[name]] <- lm(as.formula(paste(dv, "~", iv)), data = raw_data_cleaned)
  }
}

```

Overleaf Tables - first, clean model list
```{r}

```


## 2.3 Compare Each Impacted Subgroup to Non-Impacted

```{r}
## 4. Compare Each Impacted Subgroup to Non-Impacted
...
make_comparison_model <- function(df, dv, subgroup_var) {
  ...
  lm(as.formula(paste(dv, "~ treat")), data = df_sub)
}

policy_vs_nonimp_models <- list()

# any impact vs no impact
for (dv in dvs) {
  comparison_models[[paste0("comp_", dv, "_impact")]] <-
...
for (dv in dvs) {
  for (iv in c("incarc", "knowincarc", "vict")) {
    ...
    policy_vs_nonimp_models[[name]] <- make_comparison_model(raw_data_cleaned, dv, iv)
  }
}
