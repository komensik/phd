...
### 1.001 Impact Counts for appendix
...
### 1.02. Diagnostics on Lack on Survey Weights
...
#----------------------------
# 1. OPTIONAL: create White
#----------------------------
# This assumes Black, Hispanic, and OtherRace are 0/1 indicators and that
# "White" means not coded as any of those categories.
# If you already have a White variable, skip this block.

raw_data_cleaned <- raw_data_cleaned %>%
  mutate(
    White = case_when(
      is.na(Black) | is.na(Hispanic) | is.na(OtherRace) ~ NA_real_,
      Black == 0 & Hispanic == 0 & OtherRace == 0 ~ 1,
      TRUE ~ 0
    )
  )

#----------------------------
# 2. Define race columns/labels
#----------------------------
...
#----------------------------
# This uses all rows in the dataset as the denominator for percentages.
# If you want only respondents with nonmissing race info included in the denominator,
# change this to a filtered dataset first.

total_n <- nrow(raw_data_cleaned)

#----------------------------
# 5. Build impact x race table
#----------------------------
...
"sanity check" for coding -- how these behave
...
## 2.2A: Defining model sequences

```{r}
# ============================================================
# 3. Control bundles used for cumulative model waves
# Assumes these objects already exist in your variable script:
#   demo_controls
#   party_ideo_controls
...
## 2.2 Policy BV Dummies

```{r}
# ============================================================
# Goal 1 helper: full-sample models
# ============================================================

make_fullsample_model <- function(df, dv, iv, controls = character(0)) {
  rhs <- c(iv, controls)
...
for (wave_name in names(control_waves)) {
  ...
  for (dv in policy_vars) {
    for (iv in impact_vars) {
      ...
      model_list[[model_name]] <- make_fullsample_model(
        df = raw_data_cleaned,
        dv = dv,
        iv = iv,
...
##2.3 Appendix Tables - Each IV Model Grouped around DV
...
# ------------------------------------------------------------
# sanity checks: make sure the variables referenced in your
...
## 2.4 Compare Each Impacted Subgroup to Non-Impacted
