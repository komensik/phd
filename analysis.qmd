...
density_compare <- function(df, subgroup_var, label, xvar) {
  ...
  plot_df <- df %>%
    ...
    filter(!is.na(group), !is.na(d))

  means_df <- plot_df %>%
    group_by(group) %>%
    summarise(mean_val = mean(d, na.rm = TRUE))

  ggplot(plot_df, aes(x = d, color = group)) +
    geom_density(linewidth = 1.2) +
    geom_vline(data = means_df,
               aes(xintercept = mean_val, color = group),
               linetype = "dashed") +
    labs(x = "Deservingness", y = "Density", color = NULL) +
    theme_minimal()
}
#scale across outcomes
targets <- c(
