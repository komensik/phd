...
make_restricted_panel_latex_table <- function(
  ...
  # Section header: carceral contact
  body_lines <- c(
    body_lines,
    paste(
      c(
  ...
  # Carceral contact rows
  for (iv in row_vars) {
    row_label <- impact_labels[[iv]]

    model2_cells <- sapply(dv_vec, function(dv) {
      make_comp_cell(model2_list, dv, iv, robust = robust)
    })

    model3_cells <- sapply(dv_vec, function(dv) {
      make_comp_cell(model3_list, dv, iv, robust = robust)
    })
    ...
    body_lines <- c(body_lines, paste0(this_line, " \\\\"))
  }

  # Placeholder future control rows
  if (include_placeholder_control_rows) {

    placeholder_sections <- list(
      "Demographics" = c("SES", "Black", "Hispanic", "Other race", "Male"),
      "Political Identity" = c("Republican", "Democrat", "Ideology"),
      "Attitudinal Factors" = c("Core deservingness orientation",
                                "Policy disposition index",
                                "Racial resentment")
    )

    for (sec_name in names(placeholder_sections)) {

      sec_header <- paste(
        c(paste0("\\textit{", latex_escape(sec_name), "}"),
          rep("", total_data_cols)),
        collapse = " & "
      )

      body_lines <- c(body_lines, "\\addlinespace")
      body_lines <- c(body_lines, paste0(sec_header, " \\\\"))

      for (row_name in placeholder_sections[[sec_name]]) {
        blank_line <- paste(
          c(latex_escape(row_name), rep("", total_data_cols)),
          collapse = " & "
        )
        body_lines <- c(body_lines, paste0(blank_line, " \\\\"))
      }
    }
  }
...
results_table_tex <- make_restricted_panel_latex_table(
  ...
  model3_list = NULL,   # blank for now
  caption_text = "OLS Models of Carceral Policy Orientations",
  label_text = "tab:carceral_policy_orientations",
  model2_title = "Model 2",
  model3_title = "Model 3",
  row_vars = c("impact", "incarc", "knowincarc", "vict"),
  include_placeholder_control_rows = TRUE,
...
writeLines(results_table_tex, "results_policy_orientations_table.tex")
