...
# check that the 'here' root is what you expect
if ("here" %in% installed.packages()) {
  here::here()
}

# check that key files exist relative to project root
list.files("data/cleaned", pattern = "cleaned_light", recursive = TRUE)
...
-   Q: does any carceral contact or victimization change perceived deservingness of incarcerated people (`d_incwomen_abc`, 0–100 scale) compared to those without any such contact (`impact = 0`).
...
m2s <- lm(d_incwomen_abc ~ impact + Republican + Democrat + fiscal_ideology +
...
library(broom)
y_sd <- sd(raw_data_cleaned$d_incwomen_abc, na.rm = TRUE)
...
minc_s <- lm(d_incwomen_abc ~ incarc + Republican + Democrat + fiscal_ideology +
         social_ideology + penalpoint + desor_core + fire_rare + fire_privilege +
         fire_angry + fire_fear + gender + Black + Hispanic + OtherRace,
         data = raw_data_cleaned)
...
inc_sd <- sd(raw_data_cleaned$d_incwomen_abc, na.rm = TRUE)
...
for (v_vict in controls_vict) {
  f_vict <- as.formula(paste("d_incwomen_abc ~", v_vict))
...
#first make base
mv_base <- lm(d_incwomen_abc ~ vict, data=raw_data_cleaned)

for(v_v in controls_vict[-1])
{
  f_v <- as.formula(paste("d_incwomen_abc ~ vict +", v_v))
...
mvs <- lm(d_incwomen_abc ~ vict + Republican + Democrat + fiscal_ideology +
         social_ideology + penalpoint + desor_core + fire_rare + fire_privilege +
         fire_angry + fire_fear + gender + Black + Hispanic + OtherRace,
         data = raw_data_cleaned)
...
# 1. Standardized coefficients
library(broom)
y_sd_v <- sd(raw_data_cleaned$d_incwomen_abc, na.rm = TRUE)
...
for (v_prox in controls_prox) {
  f_prox <- as.formula(paste("d_incwomen_abc ~", v_prox))
...
#NEXT -- testing when adding controls in to m1 sequentially--needs fix
#first make base
mki_base <- lm(d_incwomen_abc ~ knowincarc, data=raw_data_cleaned)

for(v_prox in controls_prox[-1])
{
  f_prox <- as.formula(paste("d_incwomen_abc ~ knowincarc +", v_prox))
  fm_prox <- lm(f_prox, data = raw_data_cleaned)
...
```{r}
mkis <- lm(d_incwomen_abc ~ knowincarc + Republican + Democrat + fiscal_ideology +
         social_ideology + penalpoint + desor_core + fire_rare + fire_privilege +
         fire_angry + fire_fear + gender + Black + Hispanic + OtherRace,
         data = raw_data_cleaned)
...
# 1. Standardized coefficients
library(broom)
y_sd_prox <- sd(raw_data_cleaned$d_incwomen_abc, na.rm = TRUE)
...
-   Mediator: deservingness scores: d_incwomen_abc
...
m3 <- lm(prisonhelp ~ impact + d_incwomen_abc +
...
m4 <- lm(prisonpen ~ impact + d_incwomen_abc +
    Republican + Democrat + fiscal_ideology + social_ideology +penalpoint + desor_core + fire_rare + fire_privilege + fire_angry + fire_fear + gender + Black + Hispanic + OtherRace,
...
m5 <- lm(prisonhelp ~ d_incwomen_abc * impact + Republican + Democrat + fiscal_ideology + social_ideology +penalpoint + desor_core + fire_rare + fire_privilege + fire_angry + fire_fear + gender + Black + Hispanic + OtherRace, 
            data = raw_data_cleaned)
