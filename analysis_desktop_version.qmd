...
### Note to self: use prisonpen and prison help as indices in analysis. You made these in variable pre. Later, you're going to reverse code prisonpen, too – you should do this later in variable prep and then rerun (march 13 2026). The point is that way higher items will mean less supp
...
# items to include in the table
# NOTE:
# - prisonhelp: higher = more support for rehabilitative/help-oriented policies
# - prisonpen:  higher = less support for punitive policies ONLY AFTER reversal in variable prep
#   For now, if not yet reversed, higher = less support? No: use your current coding carefully in label.
policy_items <- c(
  "prisonhelp",
  "prisonpen",
  "q25_2_rev",
...
# labels for the final table
policy_labels <- c(
  prisonhelp = "Help-oriented policy index",
  prisonpen  = "Punitive policy index",
  q25_2_rev  = "Voting rights after serving felony sentences",
  q25_3_rev  = "Ensuring eligible incarcerated people can vote",
  q25_4_rev  = "Funding GED and college courses in prisons",
  q25_6_rev  = "Sentencing alternatives for parents of young children",
  q25_1      = "Death penalty for people convicted of murder",
  q25_5      = "Life without parole sentences"
)
...
Overleaf Table Version

```{r}
library(dplyr)
library(tibble)

deservingness_items <- c(
  "d_incppl_pris_abc",
  "d_crim_abc",
  "d_incwomen_abc"
)

deservingness_labels <- c(
  d_incppl_pris_abc = "Incarcerated People / Prisoners",
...
make_pctdiff_row <- function(df, var) {
  ...
  tibble(
    target_group = deservingness_labels[[var]],
    not_impacted_mean = round(ni_mean, 2),
    directly_impacted = paste0(round(direct_pct, 1), star_if_sig(direct_p)),
    proximally_impacted = paste0(round(prox_pct, 1), star_if_sig(prox_p)),
    victimized = paste0(round(vict_pct, 1), star_if_sig(vict_p))
...
deservingness_table <- bind_rows(
  lapply(deservingness_items, function(v) make_pctdiff_row(raw_data_cleaned, v))
)

deservingness_table

latex_rows <- deservingness_table |>
  mutate(
    row = paste0(
      target_group, " & ",
      not_impacted_mean, " & ",
...
Table as percent change for impacted groups
...
# policy items (all coded so higher = more help-oriented / less punitive)
policy_items <- c(
  "q25_1_rev",
  "q25_2_rev",
  "q25_3_rev",
...
# labels for the final table
policy_labels <- c(
  q25_1_rev = "Death penalty for people convicted of murder",
  q25_2_rev = "Voting rights after serving felony sentences",
  q25_3_rev = "Ensuring eligible incarcerated people can vote",
  q25_4_rev = "Funding GED and college courses in prisons",
  q25_5_rev = "Life without parole sentences",
  q25_6_rev = "Sentencing alternatives for parents of young children"
)
