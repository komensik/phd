...
#recode Q25_1 and Q25_5:  -99 (na_if converts to numeric)
raw_data_cleaned <- raw_data_cleaned %>%
  mutate(q25_1 = na_if(as.numeric(q25_1), -99),
         q25_5 = na_if(as.numeric(q25_5), -99)
                )
         
# Code prisonpen subitems (dpen, lwop),  "Don't know" becomes midpoint
# Create cleaned subcomponents
raw_data_cleaned <- raw_data_cleaned %>%
  mutate(
    prisonpen_1 = case_when(
      q25_1 %in% 1:4 ~ q25_1,
      q25_1 == 5 ~ 2.5,
      TRUE ~ NA_real_
    ),
    prisonpen_5 = case_when(
      q25_5 %in% 1:4 ~ q25_5,
      q25_5 == 5 ~ 2.5,
      TRUE ~ NA_real_
    )
...
raw_data_cleaned <- raw_data_cleaned %>%
  rowwise() %>%
  mutate(prisonpen = mean(c_across(c(prisonpen_1, prisonpen_5)), na.rm = TRUE)) %>%
  ungroup()
...
#prisonpen, prisonpen_1, prisonpen_2

# Quick look
psych::describe(raw_data_clean$prisonpen)
...
all_election_items <- c("auto_reg", "post_felony_vote", "early_vote", "election_holiday", "voter_id", "noncit_local", "jail_access")

#recode -99 and ensure numeric
raw_data_cleaned <- raw_data_cleaned %>%
  mutate(across(all_of(all_election_items), ~ na_if(as.numeric(.), -99)))
...
#the variables TO USE are now auto_reg_rev, xyz_rev

# save just the means of all election items as all_election_items

raw_data_cleaned <- raw_data_cleaned %>% 
  rowwise() %>% #dplyr should treat each row as own group so c_across() can return row level vector of cols for that row
  mutate(all_election_items = mean(c_across(c(auto_reg_rev, post_felony_vote_rev, early_vote_rev, election_holiday_rev, voter_id_rev, noncit_local_rev, jail_access_rev)), na.rm = TRUE)) %>%
  ungroup()

summary(raw_data_cleaned$all_election_items)

#all_election_items now exists as mean across all 7 Q23 questions, reverse coded, for each observation. 

#however, the thing that you've been reporting (as of 08-26) are just two of the questions -- post_felony_vote_rev and jail_access_rev.

# old: q23_help_items <- c("q23_2", "q23_7")
# new: incarc_voting_items

raw_data_cleaned <- raw_data_cleaned %>% 
  rowwise() %>% #dplyr should treat each row as own group so c_across() can return row level vector of cols for that row
  mutate(incarc_voting_items = mean(c_across(c(post_felony_vote_rev, jail_access_rev)), na.rm = TRUE)) %>%
  ungroup()

summary(raw_data_cleaned$incarc_voting_items)
# new saved: incarc_voting_items as the means per observation of just the two election incarceration ones, kept to oppose/IDK/support 1-2-3 scale. 

# next, to use prisonhelp, still SCHMEAR these join to prisonhelp:

incarc_voting_items <- c("post_felony_vote_rev", "jail_access_rev")

# # Rescale the 1–3 items to 1–4 range
raw_data_cleaned <- raw_data_cleaned %>%
  mutate(across(
    all_of(incarc_voting_items),
    ~ case_when(
      . == 3 ~ 4,
      . == 1 ~ 1,
      . == 2 ~ 2.5,
...
raw_data_cleaned <- raw_data_cleaned %>%
  mutate(across(c(post_felony_vote_rev_1_to_4, jail_access_rev_1_to_4),
                ~ scales::rescale(., to = c(1, 4))))

``` 

Now this is ready for prisonhelp

```{r, electionhelpqs}
# # now -- prisonhelp composite index (average of 6 items)
# 
# # rev gives reverse version of the arguments
raw_data_cleaned <- raw_data_cleaned %>%
  rowwise() %>%  # tells dplyr to treat eaach row as own group so c_across() can return row level vector of cols for that row
  mutate(prisonhelp = mean(c_across(c(q25_2_rev, q25_3_rev, q25_4_rev, q25_6_rev, post_felony_vote_rev_1_to_4, jail_access_rev_1_to_4)), na.rm = TRUE)) %>%
  ungroup()
