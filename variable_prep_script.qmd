...
#### D.4_A 'Help-Oriented' Prison Policy.
...
#### D.4_B 'Help-Oriented' Prison/Election Items (Adding to Above)
...
q23_help_items <- c("q23_2", "q23_7")

#recode -99 and ensure numeric
raw_data_cleaned <- raw_data_cleaned %>%
  mutate(across(all_of(q23_help_items), ~ na_if(as.numeric(.), -99)))

#recode 1-3 scale to match direction of prison help items 
# here, 1 = favor, 2 = oppose, 3 = DK
# we want higher = more support for help-oriented policy so 1-> 3, 2-> 1, 3 -> NA
raw_data_cleaned <- raw_data_cleaned %>%
  mutate(across(
    all_of(q23_help_items),
    ~ case_when(
      . == 1 ~ 4,
      . == 2 ~ 1,
      . == 3 ~ 2.5,
...
# Rescale the 1–3 items (after reversing) to 1–4 range
raw_data_cleaned <- raw_data_cleaned %>%
  mutate(across(c(q23_2_rev, q23_7_rev),
...
raw_data_cleaned <- raw_data_cleaned %>%
  ...
  mutate(prisonhelp = mean(c_across(c(q25_2_rev, q25_3_rev, q25_4_rev, q25_6_rev, q23_2_rev, q23_7_rev)), na.rm = TRUE)) %>%
  ungroup()
