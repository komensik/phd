...
policy_table <- purrr::map_dfr(policy_map$var, ~ policy_summary(sample, .x)) %>%
...
m_a <- lm(min_wage ~ pid, data = sample) # given someone's PID, how much min wage support would we predict



m_b <- lm(impact ~ pid, data = sample)  # given unit's pid, how much carceral impact would we predict? 
...
covs_min_wage_1 <- lm(min_wage ~pid + self_ideo + desor_core + White + Black + Hisp_Latino + Asian_Am + Native_Am + OtherRace + gov_attn, data = sample)
...
#sample %>% count(d_incppl_pris_abc)

#summary:
#summary(sample$d_incppl_pris_abc)



#histogram
incarc_ppl_hist <- hist(sample$d_incppl_pris_abc, breaks = 20,
     main = "Deservingness of Incarcerated People/Prisoners",
     xlab = "Deservingness (0–100)", col = "gray80", border = "white")


#mean(sample$d_incppl_pris_abc, na.rm = TRUE)

#sd(sample$d_incppl_pris_abc, na.rm = TRUE)

#var(sample$d_incppl_pris_abc, na.rm = TRUE)

boxplot(sample$d_incppl_pris_abc)

#density plot
dv_density_data <- sample %>%
...
desor_core_hist <- hist(sample$desor_core, breaks = 20,
...
sd(sample$desor_core, na.rm = TRUE)
...
```{r}
m1 <- lm(d_incppl_pris_abc ~ impact, data = sample)
summary(m1)
...
m1c <- lm(d_incppl_pris_abc ~ impact +
    ...
    gender + Black + Hisp_Latino + OtherRace,
  data = sample
)
...
qqnorm(sample$d_incppl_pris_abc)
```
...
m1cs <- lm(d_incppl_pris_abc ~ impact + Republican + Democrat + fiscal_ideology +
         ...
         fire_angry + fire_fear + gender + Black + Hisp_Latino + OtherRace,
         data = sample)

# 1. Standardized coefficients
library(broom)
y_sd <- sd(sample$d_incppl_pris_abc, na.rm = TRUE)
...
controls <- c("impact", "Republican","Democrat","fiscal_ideology","social_ideology","penalpoint","desor_core","fire_rare", "fire_privilege","fire_angry","fire_fear","gender","Black","Hispanic/Latino","OtherRace")
...
table(sample$q62_1, sample$penalpoint, useNA = "ifany")

library(ggplot2)

# Convert to a data frame for ggplot
tab_df <- as.data.frame(table(
  Original = sample$q62_1,
  Reversed = sample$penalpoint
))
...
mean(sample$q62_1, na.rm = TRUE)
mean(sample$penalpoint, na.rm = TRUE)

cor(sample$q62_1, sample$penalpoint, use = "pairwise.complete.obs")

par(mfrow = c(1,2))
hist(sample$q62_1, main="Original")
hist(sample$penalpoint, main="Reversed")

par(mfrow = c(1,2))
hist(sample$q62_1, main="Original (Q62_1)")
hist(8 - sample$q62_1, main="What a Reverse Looks Like")
...
inc_sd <- sd(sample$d_incppl_pris_abc, na.rm = TRUE)
...
controls_inc <- c("incarc", "Republican","Democrat","fiscal_ideology","social_ideology","penalpoint","desor_core","fire_rare", "fire_privilege","fire_angry","fire_fear","gender","Black","Hispanic/Latino","OtherRace")
  
results_inc <- data.frame() #to reset

for (v_inc in controls_inc) {
  f_inc <- as.formula(paste("d_incppl_pris_abc ~", v_inc))
  fm_inc <- lm(f_inc, data = sample)
...
minc_base <- lm(d_incppl_pris_abc ~ incarc, data = sample)
...
# 1. Standardized coefficients
library(broom)
y_sd_v <- sd(sample$d_incppl_pris_abc, na.rm = TRUE)
...
controls_vict <- c("vict", "Republican","Democrat","fiscal_ideology","social_ideology","penalpoint","desor_core","fire_rare", "fire_privilege","fire_angry","fire_fear","gender","Black","Hispanic/Latino","OtherRace")

results_vict <- data.frame()

for (v_vict in controls_vict) {
  f_vict <- as.formula(paste("d_incppl_pris_abc ~", v_vict))
  fm_vict <- lm(f_vict, data = sample)
...
# 1. Standardized coefficients
library(broom)
y_sd_prox <- sd(sample$d_incppl_pris_abc, na.rm = TRUE)
...
controls_prox <- c("knowincarc","Republican","Democrat","fiscal_ideology","social_ideology","penalpoint","desor_core","fire_rare", "fire_privilege","fire_angry","fire_fear","gender","Black","Hispanic/Latino","OtherRace")

results_prox <- data.frame()

for (v_prox in controls_prox) {
  f_prox <- as.formula(paste("d_incppl_pris_abc ~", v_prox))
  fm_prox <- lm(f_prox, data = sample)
...
#NEXT -- testing when adding controls in to m1 sequentially--needs fix
#first make base
mki_base <- lm(d_incppl_pris_abc ~ knowincarc, data=sample)
...
key_vars <- c("d_incppl_pris_abc","incarc","knowincarc","vict","desor_core",
              ...
              "fire_angry","fire_fear","gender","Black","Hispanic/Latino","OtherRace")
...
ncheck_inc <- sapply(controls_inc, function(v) 
  nobs(lm(as.formula(paste("d_incppl_pris_abc ~", v)), data = sample))
) |> 
...
```{r, controls vector}
var(sample$prisonhelp, na.rm=TRUE)

names(sample)

m3_0 <- lm(prisonhelp ~ Republican + Democrat , data = sample)
...
```{r}
prisonhelp_by_party <- lm(prisonhelp ~ Republican + Democrat, data = sample)
prisonpen_by_party <- lm(prisonpen ~ Republican + Democrat, data = sample)
...
```{r}
prisonhelp_by_penalpoint <- lm(prisonhelp ~ penalpoint, data = sample)
prisonpen_by_penalpoitn <- lm(prisonpen ~ penalpoint, data = sample)

prison_help_by_fire_rare <- lm(prisonhelp ~ fire_rare, data = sample)
prison_pen_by_fire_rare <- lm(prisonpen ~ fire_rare, data = sample)

prison_help_by_fire_privilege <- lm(prisonhelp ~ fire_privilege, data = sample)
prison_pen_by_fire_privilege <- lm(prisonpen ~ fire_privilege, data = sample)

prison_help_by_fire_angry <- lm(prisonhelp ~ fire_angry, data = sample)
prison_pen_by_fire_angry <- lm(prisonpen ~ fire_angry, data = sample)

prison_help_by_fire_fear <- lm(prisonhelp ~fire_fear, data = sample)
prison_pen_by_fire_fear <- lm(prisonpen ~fire_fear, data = sample)
...
mh1_no_cont <- lm(min_wage ~ impact, data = sample)
mh2_no_cont <- lm(free_calls ~ impact, data = sample)
mh3_no_cont <- lm(ed_funds ~ impact, data = sample)
mh4_no_cont <- lm(alt_sentencing ~ impact, data = sample)
mh5_no_cont <- lm(q23_2_rev ~ impact, data = sample)
mh6_no_cont <- lm(q23_7_rev ~ impact, data = sample)




mpen1_no_control <- lm(prisonpen_1 ~ impact, data = sample)

mpen2_no_control <- lm(prisonpen_5 ~ impact, data = sample)
...
#character vector of controls vector for m3 m4
 m3m4controls <- c("d_incppl_pris_abc",
  ...
  "fire_fear", "gender", "Black", "Hispanic/Latino", "OtherRace"
)
...
#M3 M4 OLS Models
m3_basic <- lm(m3_controls_formula, data = sample)
m4_basic <- lm(m4_controls_formula, data = sample)
```
...
base_m3 <- lm(prisonhelp ~ d_incppl_pris_abc + impact, data = sample)
full <- update(base_m3, . ~ . + desor_core)
summary(full)$adj.r.squared - summary(base_m3)$adj.r.squared

base_m4 <- lm(prisonpen ~ d_incppl_pris_abc + impact, data = sample)
...
hist(sample$prisonhelp)

hist(sample$prisonpen)
...
m3 <- lm(prisonhelp ~ impact + d_incppl_pris_abc +
    Republican + Democrat + fiscal_ideology + social_ideology +penalpoint + desor_core + fire_rare + fire_privilege + fire_angry + fire_fear + gender + Black + Hisp_Latino + OtherRace,
...
```{r}
prisonpen_by_party <- lm(prisonhelp ~ Republican + Democrat, data = sample)
summary(prisonpen_by_party)
```
...
```{r}
cor(sample$fire_angry, sample$prisonpen, use="pairwise.complete.obs")
...
m4_inc <- lm(prisonpen ~ incarc + d_incppl_pris_abc +
    Republican + Democrat + fiscal_ideology + social_ideology +penalpoint + desor_core + fire_rare + fire_privilege + fire_angry + fire_fear + gender + Black + Hisp_Latino + OtherRace,data = sample)
...
m5 <- lm(prisonhelp ~ d_incppl_pris_abc * impact + Republican + Democrat + fiscal_ideology + social_ideology + penalpoint + desor_core + fire_rare + fire_privilege + fire_angry + fire_fear + gender + Black + Hisp_Latino + OtherRace, 
            data = sample)
...
m5_c<-lm(prisonhelp ~ d_inc_c*impact + Republican + 
    ...
    gender + Black + Hisp_Latino + OtherRace, data = x_data)
...
m6 <- lm(prisonpen ~ d_incppl_pris_abc * impact + Republican + Democrat + fiscal_ideology + social_ideology + penalpoint + desor_core + fire_rare + fire_privilege + fire_angry + fire_fear + gender + Black + Hisp_Latino + OtherRace, 
            data = sample)
...
```{r}
summary(is.na(sample$impact))
summary(is.na(sample$d_incppl_pris_abc))
summary(is.na(sample$prisonhelp))
summary(is.na(sample$prisonpen))

sum(complete.cases(sample[, c("impact", "d_incppl_pris_abc", "prisonhelp", "prisonpen","Republican","Democrat","fiscal_ideology","social_ideology","penalpoint","desor_core","fire_rare", "fire_privilege","fire_angry","fire_fear","gender","Black","Hispanic/Latino","OtherRace")]))
...
table(sample$impact, sample$complete_case)
prop.table(table(sample$impact, sample$complete_case), 1)
#chisq to test whether being a complete case is independent of treatment status, get yes/I can reject the null
chisq.test(table(sample$impact, sample$complete_case))
...
#NOTE: using rescaled jail and post felony ("1 to 4"). For non, drop 1 to 4)
help_policy_items <- c("min_wage", "free_calls", "ed_funds", "alt_sentencing", "post_felony_vote_rev_1_to_4", "jail_access_rev_1_to_4")

# Check distributions and means
summary(x_data[, c("min_wage","free_calls","ed_funds","alt_sentencing","post_felony_vote_rev_1_to_4","jail_access_rev_1_to_4")])
cor(x_data$post_felony_vote_rev_1_to_4, x_data$jail_access_rev_1_to_4, use="pairwise.complete.obs")

# checking if variance changes much when excluding q23s
x_data <- x_data %>%
  rowwise() %>%
  mutate(var_help_no_q23 = var(c_across(c("min_wage","free_calls","ed_funds","alt_sentencing")), na.rm = TRUE))
...
library(e1071)
skewness(sample$variancehelp, na.rm = TRUE)
skewness(sample$variancehelp_log, na.rm = TRUE)
skewness(sample$sdhelp, na.rm = TRUE)
```
